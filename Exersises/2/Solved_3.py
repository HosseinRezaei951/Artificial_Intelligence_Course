...
for i in range(num_iter):
    ...
    #gradient = (-np.dot(x_train.T, (h - y_train))/ y_train.size)+ 2*theta*Regulization_lamda
...
for i in range(num_iter):
    ...
    #gradient = -np.dot(x_train.T, (h - y_train)) / y_train.size
    gradient = (-np.dot(x_train.T, (h - y_train))/ y_train.size)+ 2*theta*Regulization_lambda
    theta = theta+ lr * gradient
